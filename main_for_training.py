...
if __name__ == "__main__":
    ...
    if not os.path.isfile(modelfile) and  args.task != "training":
    ...
    with tf.Session(config=config) as sess:
        if args.task == "training":
            ...
            file_name ="/"+args.model+"_"+str(4)+"testlayer_"+args.activation+"_"+args.layer_parameters[0]
            ##else:
              ##  file_name = args.modelfile
            train_ret = train(data, file_name=file_name, params=args.layer_parameters, num_epochs=args.epochs, lr=args.lr, decay=args.wd, activation=args.activation, activation_param=args.leaky_slope, grad_reg=args.gradreg, dropout_rate = args.dropout)
            import sys
            print(f'training is done! exit')
            sys.exit(0)
